...
<xsl:preserve-space elements="tei:c tei:rdg"/>
  <xsl:template match="/">
	<html>
	<head>
    ...
    <xsl:choose>
      ...
      <xsl:when test="(ancestor::tei:q[1]//tei:w|ancestor::tei:q[1]//tei:pc)/@xml:id = ./@xml:id and count($texta)">
        ...
        <xsl:when test="$texta[contains(@target,$q1)]"> 
          ...
          <xsl:if test="count($emend) and $emend[contains(@target,$q1)]"> 
            <img src="fdt-emend-l.png" class="imgEmend" alt="{$emendDesc}" title="{$emendDesc}"/>
          </xsl:if>
          <xsl:call-template name="openQuote"/>
          <xsl:if test="count($emend) and not($emend[contains(@target,$q1)])"> 
            <img src="fdt-emend-l.png" class="imgEmend" alt="{$emendDesc}" title="{$emendDesc}"/>
          </xsl:if>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="tei:c">
    ...
    <xsl:when test="key('altRdg',@xml:id)">
      ...
      <xsl:when test="//tei:app[contains(@from,$xmlId)]/tei:rdg[contains(@wit,$rdg)]">
        <span title="{./@n}" class="{$class}">
          <xsl:variable name="c" select="//tei:app[contains(@from,$xmlId)]/tei:rdg[contains(@wit,$rdg)]"/>
          <xsl:value-of select="translate($c,' ','&#160;')"/>
        </span>
      </xsl:when>
      <xsl:otherwise>

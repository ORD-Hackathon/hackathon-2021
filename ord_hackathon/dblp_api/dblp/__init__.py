...
DBLP_AUTHOR_SEARCH_URL_ORCID = DBLP_BASE_URL + 'search/orcid'
...
def searchORCID(author_str):
    resp = requests.get(DBLP_AUTHOR_SEARCH_URL, params={'orcid':author_str})
    #TODO error handling
    root = etree.fromstring(resp.content)
    return [Author(urlpt) for urlpt in root.xpath('/orcid/orcid/@urlpt')]
